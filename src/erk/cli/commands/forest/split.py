...
def split_forest(
    ...
    # Get current branch
    current_branch = ctx.git_ops.get_current_branch(ctx.repo.root)
    ...
    # Get stack
    stack_branches = ctx.graphite_ops.get_branch_stack(ctx.git_ops, ctx.repo.root, current_branch)
    ...
    existing_worktrees = ctx.git_ops.list_worktrees(ctx.repo.root)
    ...
    for branch in plan.branches_to_split:
        ...
        unique_name = ensure_unique_worktree_name(base_name, worktrees_dir, ctx.git_ops)
