...
def test_switch_up_with_existing_worktree() -> None:
    ...
    with simulated_workstack_env(runner) as env:
        ...
        # Mock Path.cwd() to return feature-1 worktree path
...
def test_switch_up_with_mismatched_worktree_name() -> None:
    ...
    with simulated_workstack_env(runner) as env:
        ...
        db_refactor_path = env.create_linked_worktree("db-refactor", "feature/db", chdir=False)
        ...
        git_ops, graphite_ops = env.build_ops_from_branches(
            {
                "main": BranchMetadata.main(children=["feature/db"], sha="abc123"),
                "feature/db": BranchMetadata.branch(
                    "feature/db", children=["feature/db-tests"], sha="def456"
                ),
                "feature/db-tests": BranchMetadata.branch(
                    "feature/db-tests", parent="feature/db", sha="ghi789"
                ),
            },
        ...
        # Mock Path.cwd() to return feature/db worktree path
        from unittest import mock

        with mock.patch("pathlib.Path.cwd", return_value=db_refactor_path):
            result = runner.invoke(
                cli, ["switch", "--up", "--script"], obj=test_ctx, catch_exceptions=False
            )
...
def test_switch_down_with_mismatched_worktree_name() -> None:
    ...
    with simulated_workstack_env(runner) as env:
        ...
        git_ops, graphite_ops = env.build_ops_from_branches(
            {
                "main": BranchMetadata.main(children=["feature/api"], sha="abc123"),
                "feature/api": BranchMetadata.branch(
                    "feature/api", children=["feature/api-v2"], sha="def456"
                ),
                "feature/api-v2": BranchMetadata.branch(
                    "feature/api-v2", parent="feature/api", sha="ghi789"
                ),
            },
        ...
        # Mock Path.cwd() to return feature/api-v2 worktree path
        from unittest import mock

        with mock.patch("pathlib.Path.cwd", return_value=api_v2_work_path):
            result = runner.invoke(
                cli, ["switch", "--down", "--script"], obj=test_ctx, catch_exceptions=False
            )
