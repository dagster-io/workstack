...
from dot_agent_kit.integrations.github_cli import DotAgentGitHubCli
from erk.core.github.issues import GitHubIssues


def require_github_cli(ctx: click.Context) -> DotAgentGitHubCli:
    """Get GitHub CLI from context, exiting with error if not initialized.
    ...
    Returns:
        DotAgentGitHubCli instance from context
    ...
    Example:
        ...
        ...     result = github_cli.create_issue(title, body, labels)
    """
    if ctx.obj is None:
        click.echo("Error: Context not initialized", err=True)
        raise SystemExit(1)

    return ctx.obj.github_cli
