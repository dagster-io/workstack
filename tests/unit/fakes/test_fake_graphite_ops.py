...
def test_fake_graphite_ops_branch_hierarchy() -> None:
    ...
    branches = {
        "main": BranchMetadata.main(children=["level-1"], sha="abc123"),
        "level-1": BranchMetadata.branch("level-1", children=["level-2"], sha="def456"),
        "level-2": BranchMetadata.branch("level-2", parent="level-1", sha="ghi789"),
    }
    ops = FakeGraphiteOps(branches=branches)
    git_ops = FakeGitOps()
...
def test_fake_graphite_ops_combined_config() -> None:
    ...
    branches = {
        "main": BranchMetadata.main(children=["feat-a"], sha="abc123"),
        "feat-a": BranchMetadata.branch("feat-a", sha="def456"),
    }
    stacks = {
