...
def execute_land_branch(
    git: Git | None = None,
    github: GitHub | None = None,
    graphite: Graphite | None = None,
    repo_root: Path | None = None,
) -> LandBranchSuccess | LandBranchError:
    ...
    parent = graphite.get_parent_branch(git, repo_root, branch_name)
    ...
    trunk = git.get_trunk_branch(repo_root)
    if parent != trunk:
        return LandBranchError(
            success=False,
            error_type="parent_not_trunk",
    ...
    children = graphite.get_child_branches(git, repo_root, branch_name)

    # Step 6: Merge the PR
    try:
        github.merge_pr(repo_root, pr_number, squash=True)
    except RuntimeError:
        return LandBranchError(
            success=False,
    ...
    child_branch = None
    if len(children) == 1:
        if graphite.navigate_to_child(repo_root):
