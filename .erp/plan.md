...
# Refactor Exception Handling and Fix Kit CLI Command Names

## Overview

This plan addresses two related cleanup tasks:
...
Both tasks improve code consistency and reduce technical debt.

---

## Part 1: Refactor Try-Catch Patterns in create_cmd.py
...
## Part 2: Fix Kit CLI Command Function Name Mismatches

### Problem Statement

Two kit CLI commands have function names that don't match the kit loader's expectations, causing warnings on every `dot-agent run erk` invocation:

```
Warning: Command 'plan-save-from-session' in kit 'erk' does not have expected function 'plan_save_from_session'
Warning: Command 'agent-debug' in kit 'erk' does not have expected function 'agent_debug'
```

### Root Cause

The kit CLI loader expects function names to match command names (converted to snake_case):

| Command Name | Expected Function | Actual Function | File |
|--------------|------------------|-----------------|------|
| `plan-save-from-session` | `plan_save_from_session()` | `save_plan_from_session()` | `plan_save_from_session.py` |
| `agent-debug` | `agent_debug()` | `debug_agent()` | `agent_debug.py` |

### Implementation

#### Fix 1: Rename `save_plan_from_session` → `plan_save_from_session`

**File:** `/packages/dot-agent-kit/src/dot_agent_kit/data/kits/erk/kit_cli_commands/erk/plan_save_from_session.py`

**Line 81:** Change function definition:
```python
# Before
...
**Line 44:** Update function call in `_format_success_display()` if needed (check if this helper calls the main function).

#### Fix 2: Rename `debug_agent` → `agent_debug`

**File:** `/packages/dot-agent-kit/src/dot_agent_kit/data/kits/erk/kit_cli_commands/erk/agent_debug.py`

**Around line 380:** Change function definition (where `@click.command(name="agent-debug")` decorator is):
```python
# Before
def debug_agent(
    session_id: str | None,
...
dot-agent run erk --help 2>&1 | grep Warning

# Should output nothing (no warnings)

# Test commands still work
dot-agent run erk agent-debug --help
dot-agent run erk plan-save-from-session --help
```

### Impact

**Before:**
- Warnings appear on every `dot-agent run erk` command
- Cluttered output makes real errors harder to spot
...
**Part 1:**
- ✅ All 3 new Ensure methods added
- ✅ All 13+ unit tests pass
- ✅ 4 try-catch blocks converted in create_cmd.py
- ✅ Existing command tests pass
- ✅ Pyright reports zero new errors

**Part 2:**
- ✅ Both function names match kit loader expectations
- ✅ No warnings on `dot-agent run erk --help`
- ✅ Both commands still functional

**Overall:**
- ✅ Cleaner codebase with reduced boilerplate
...
```bash
erk implement 1282 --yolo
```

**Dangerous mode (auto-submit PR after implementation):**
```bash
erk implement 1282 --dangerous
```