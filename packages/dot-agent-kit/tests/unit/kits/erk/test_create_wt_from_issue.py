...
def test_create_from_issue_number_success(
...
def test_create_from_github_url_success(
    monkeypatch: pytest.MonkeyPatch, tmp_path: Path
) -> None:
    ...
    result = runner.invoke(
        create_wt_from_issue, ["https://github.com/owner/repo/issues/456"]
    )
...
def test_missing_erk_plan_label(
    monkeypatch: pytest.MonkeyPatch, tmp_path: Path
) -> None:
...
def test_issue_with_empty_body(
    monkeypatch: pytest.MonkeyPatch, tmp_path: Path
) -> None:
...
def test_issue_with_special_chars_in_title(
    monkeypatch: pytest.MonkeyPatch, tmp_path: Path
) -> None:
...
def test_save_issue_reference_warning(
    monkeypatch: pytest.MonkeyPatch, tmp_path: Path
) -> None:
