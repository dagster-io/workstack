"""Unit tests for debug_agent command.

Tests the debug-agent CLI command for inspecting failed agent runs.
"""
...
from erk.data.kits.erk.kit_cli_commands.erk.debug_agent import (
    debug_agent,
    discover_agent_logs,
    export_json,
...
def test_find_project_dir_for_session_success(tmp_path: Path) -> None:
    ...
    import erk.data.kits.erk.kit_cli_commands.erk.debug_agent as debug_module
...
def test_cli_with_explicit_session_id(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(
        "erk.data.kits.erk.kit_cli_commands.erk.debug_agent.Path.home",
