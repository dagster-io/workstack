...
│   │   ├── __init__.py
│   │   ├── database.py        ← Database adapter (ABC + Real)
│   │   ├── api_client.py      ← API client adapter
│   │   ├── filesystem.py      ← File system adapter
│   │   └── message_queue.py   ← Message queue adapter
...
│       ├── test_user.py
│       └── test_order.py
├── integration/               ← Tests WITH real adapters (Layer 2)
...
def test_real_database_with_mocking(monkeypatch) -> None:
    """Test real adapter with mocked connections."""
...
### Layer 2: Real Adapters with Mocking
...
## Quick Checklist: Adding a New Adapter Method

When adding a method to an adapter interface:
