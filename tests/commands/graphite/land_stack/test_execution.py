...
def test_land_stack_does_not_run_gt_sync() -> None:
    """Test that gt sync -f IS run automatically during restack phase.

    After bug fix, gt sync -f runs automatically to rebase upstack branches.
    This test verifies that graphite_ops.sync() is called with force=True.

    Rationale: Without automatic restacking, gt submit fails with
    "merged commits are not contained in trunk" error.
    """
    runner = CliRunner()
    with erk_inmem_env(runner) as env:
        ...
        assert len(graphite_ops.sync_calls) == 1, (
            f"gt sync should be called automatically during restack phase. "
            f"Expected 1 sync call, got {len(graphite_ops.sync_calls)} calls: "
            f"{graphite_ops.sync_calls}"
        )

        # Verify sync was called with force=True
...
def test_land_stack_runs_gt_sync_in_restack_phase() -> None:
    ...
    This test ensures that after landing a PR, gt sync -f is called to:
    1. Update Graphite metadata about merged branches
    2. Rebase remaining upstack branches onto new trunk state

    Without this, gt submit will fail with "merged commits are not contained in trunk".
    """
    runner = CliRunner()
    with erk_inmem_env(runner) as env:
        # Setup simple stack: main → feat-1
        # This is sufficient to test that sync is called after landing
        git_ops, graphite_ops = env.build_ops_from_branches(
            {
                "main": BranchMetadata.trunk("main", children=["feat-1"], commit_sha="abc123"),
                "feat-1": BranchMetadata.branch("feat-1", "main", commit_sha="def456"),
        ...
        # Act: Land feat-1 (no upstack branches, but sync should still be called)
        result = runner.invoke(cli, ["land-stack", "--force"], obj=test_ctx)
        ...
        for _repo_root_arg, force_arg, quiet_arg in graphite_ops.sync_calls:
            assert force_arg is True, "sync must be called with force=True"
            assert quiet_arg is True, "sync should be quiet in default mode (verbose=False)"


def test_land_stack_skips_gt_sync_with_down_flag() -> None:
    """Verify gt sync is NOT called when using --down flag.

    The --down flag provides manual control by skipping upstack rebase operations.
    When used, no restack phase should run.
    """
    runner = CliRunner()
    with erk_inmem_env(runner) as env:
        # Setup simple stack: main → feat-1
        ...
        assert len(graphite_ops.sync_calls) == 0, (
            f"With --down flag, sync should NOT be called. "
            f"Got {len(graphite_ops.sync_calls)} calls: {graphite_ops.sync_calls}"
        )
...
def test_land_stack_restack_respects_verbose_flag() -> None:
    ...
    The quiet parameter should be the inverse of verbose:
    - Without --verbose (default): quiet=True (suppress output)
    - With --verbose: quiet=False (show detailed output)
    """
    runner = CliRunner()
    ...
    with erk_inmem_env(runner) as env:
        ...
        assert result.exit_code == 0, f"Command failed: {result.output}"
        assert len(graphite_ops.sync_calls) >= 1, "Expected at least 1 sync call"
        _, _, quiet_arg = graphite_ops.sync_calls[0]
        assert quiet_arg is True, "Without --verbose, quiet should be True (verbose=False)"
    ...
    with erk_inmem_env(runner) as env:
        ...
        result = runner.invoke(
            cli, ["land-stack", "--force", "--verbose"], obj=test_ctx
        )

        assert result.exit_code == 0, f"Command failed: {result.output}"
        assert len(graphite_ops.sync_calls) >= 1, "Expected at least 1 sync call"
        _, _, quiet_arg = graphite_ops.sync_calls[0]
        assert quiet_arg is False, "With --verbose, quiet should be False (verbose=True)"
