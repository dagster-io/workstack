...
⚠️ **DEPRECATED**: This command is deprecated. Use `/erk:enhance-and-save-plan` or `/erk:save-plan` instead, which create GitHub issues directly without requiring disk files.
...
- Re-run: `/erk:create-plan-from-file --link <issue-number>`
...
2. Create issue: `/erk:create-plan-from-file` ← **YOU ARE HERE**
...
4. Create issue: `/erk:create-plan-from-file` ← Issue automatically linked to `.impl/` folder

---

## Agent Instructions

You are executing the `/erk:create-plan-from-file` command. Follow these steps carefully:
...
1. Call the composite kit command with the plan file path:
   ...
   ```bash
   result=$(dot-agent kit-command erk create-plan-from-file "<path-to-plan-file>")
