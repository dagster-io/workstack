...
This guide explains the `.plan/` folder protocol used in erk for managing implementation plans.

## Overview

Erk uses `.plan/` folders to track implementation progress for plans executed locally by agents.
...
This creates a `.plan/` folder in the current directory with the implementation plan.
...
This creates a new worktree with the `.plan/` folder copied into it.
...
The agent reads `.plan/plan.md`, executes each phase, and updates `.plan/progress.md` as steps complete.

## Progress Tracking

The `.plan/progress.md` file tracks completion status:
...
- **Never commit `.plan/` folders** - They're in `.gitignore` for a reason
