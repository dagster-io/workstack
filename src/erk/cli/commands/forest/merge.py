...
def merge_forest(
    ...
    dirty_worktrees = check_uncommitted_changes(
        plan.worktrees_to_remove, ctx.git_ops, worktrees_dir
    )
    ...
    for wt_name in plan.worktrees_to_remove:
        ...
        ctx.git_ops.remove_worktree(ctx.repo.root, wt_path, force=False)
