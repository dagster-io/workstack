...
def identify_removable_worktrees(forest: ForestData, target_worktree: str) -> list[str]:
    """Return worktrees to remove (all except target).

    Args:
        forest: ForestData to merge
...
def create_forest_merge_plan(
    forest: ForestData, target_worktree: str | None, current_worktree: str | None
) -> ForestMergePlan:
...
def check_uncommitted_changes(
    worktrees_to_check: list[str], git: Git, worktrees_dir: Path
) -> list[str]:
    ...
    for wt_name in worktrees_to_check:
        ...
        if not git.path_exists(wt_path):
            continue
