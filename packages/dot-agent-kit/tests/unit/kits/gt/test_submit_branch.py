...
""",
    ...
    def test_post_analysis_with_large_pr_body(self, tmp_path: Path) -> None:
        """Test that large PR bodies (>100KB) work correctly via stdin."""
        # Create .impl/issue.json
        impl_dir = tmp_path / ".impl"
        impl_dir.mkdir()
        issue_json = impl_dir / "issue.json"
        ...
        # Create a large commit message body (>100KB)
        large_description = "This is a test description.\n" * 10000  # ~250KB
        ...
        # Verify that the large body was successfully set
        github_ops = ops.github()
        github_state = github_ops.get_state()
        pr_body = github_state.pr_bodies[456]
        ...
        # Should contain the large description
        assert large_description in pr_body

        # Verify the body is indeed large (>100KB)
        assert len(pr_body) > 100000
