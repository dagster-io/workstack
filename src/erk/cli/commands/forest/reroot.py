...
def _handle_abort(ctx: ErkContext, repo_dir: Path) -> None:
    ...
    # Load reroot state
    state = ctx.forest.load_reroot_state()
...
def _handle_continue(ctx: ErkContext, repo_dir: Path, force: bool) -> None:
    ...
    conflicted_files = ctx.git.get_conflicted_files(worktree)
    ...
    ctx.git.commit_with_message(resolved_msg, worktree)
...
def _handle_initial_reroot(
    ...
    existing_state = ctx.forest.load_reroot_state()
    ...
    marker_path = repo_dir.parent / ".erk" / "REROOT_IN_PROGRESS"
    if ctx.git.path_exists(marker_path):
        user_output(
            click.style("❌ Error: ", fg="red")
    ...
    # Create reroot plan
    plan = create_reroot_plan(forest, ctx.graphite, ctx.git, trunk_branch, ctx.repo.root)
...
def _execute_reroot_loop(
    ...
    for idx, branch in enumerate(branches_to_process, start=1):
        ...
        # Get parent SHA
        parent_branch = ctx.graphite.get_parent_branch(ctx.git, ctx.repo.root, branch)
        ...
        parent_sha = ctx.git.get_commit_sha(parent_branch, parent_wt)

        # Rebase
        result = ctx.git.rebase_branch(branch, parent_branch, worktree)
        ...
        if result.has_conflicts:
            ...
            if should_commit:
                ...
                ctx.git.commit_with_message(conflict_msg, worktree)
                ...
                ctx.forest.save_reroot_state(state)
