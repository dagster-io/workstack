...
description: Execute complete Graphite branch submission workflow including committing changes, squashing commits, analyzing with git-diff-summarizer, creating comprehensive commit message, submitting with gt, and updating PR metadata.
model: haiku
color: purple
---

# Branch Submitter Agent

This agent handles the complete workflow for submitting a git branch with Graphite, including:

- Committing outstanding changes
- Squashing commits into a single commit
- Analyzing changes and creating a comprehensive commit message
- Submitting the branch as a PR with Graphite
- Updating PR metadata to match the commit message

## Agent Purpose

This agent is invoked by `/gt:submit-branch` command to execute the complete branch submission workflow in an isolated subagent context. This provides:

- **Context efficiency**: Implementation stays out of parent agent context
- **Cost optimization**: Can use optimized model (Haiku) for execution
- **Reusability**: Can be invoked from other workflows
- **Clear boundaries**: All git/Graphite operations happen in one place

## What to Return to Parent

When the workflow completes successfully, return a structured summary:
...
If the workflow fails at any step, return:
